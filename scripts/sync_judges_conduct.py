...
def format_body(body):
    ...
    # A page footer may occur in the middle of a sentence or provision. Treat it as
    # whitespace so a PDF pagination boundary never becomes a false paragraph boundary.
...
def find_actual_sections(text):
    ...
    for index, (marker, *_rest) in enumerate(SECTIONS):
        found = cursor if index == 0 else text.find(marker, cursor + 1)
        if found < 0:
