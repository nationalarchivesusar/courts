...
- `assets/dockets.js` reads the two configured public docket boards for current
  proceedings on the home page and the dedicated `/docket/` page. Until the
  structured Phase F case API is deployed and verified in production, District
  Court docket links deliberately remain source-card links rather than routing
  users into an unavailable JIS case endpoint.
...
- `/docket/` is the public current-proceedings surface. The current production
  version uses the public Supreme Court and District Court docket boards and
  their local fallback rather than depending on undeployed case API routes.
...
Stable public namespaces include `/docket/`, `/caselaw/`, `/records/`,
`/judges/`, and the JIS `/api/v1/` endpoints. Future backed services may add
opinions/orders without creating empty placeholder pages. The authenticated JIS
review console is an internal backend surface and does not belong in the public
Courts navigation.
